  ...
  private eventListener(popupWindow: Window, redirectUri = getWindowOrigin()) {
    ...
    return merge(
      ...
      switchMap((event: Event) => {
        ...
        const url = (<any>event)['url'] as string;

        if (redirectUri && url && url.indexOf(redirectUri) !== 0) {
          return EMPTY;
        }
  ...
  private pollPopup(popupWindow: Window, redirectUri = getWindowOrigin()) {
    return interval(50).pipe(
      switchMap(() => {
        ...
        if (popupWindowOrigin && redirectUri) {
          sameOrigin = redirectUri.indexOf(popupWindowOrigin) === 0 || popupWindowOrigin.indexOf(redirectUri) === 0;
        }

        if (sameOrigin && (popupWindow.location.search || popupWindow.location.hash)) {
